...
-- ============================================================================
-- 2. refresh_tokens: token-family for replay/theft detection + indexes + uniqueness
-- ============================================================================
...
CREATE INDEX audit_entries_org_ts_idx     ON audit_entries (organization_id, ts DESC);
CREATE INDEX audit_entries_op_ts_idx      ON audit_entries (operation_id, ts DESC);
CREATE INDEX audit_entries_actor_ts_idx   ON audit_entries (actor_user_id, ts DESC);
CREATE INDEX audit_entries_entity_idx     ON audit_entries (entity_type, entity_id);
CREATE INDEX audit_entries_request_id_idx ON audit_entries (request_id);
...
-- ============================================================================
-- 4. updated_at: trigger to bump on every UPDATE (Drizzle's defaultNow runs only
--    on INSERT). Without this, sync/changefeed semantics rely on an unpopulated
--    column.
-- ============================================================================
...
CREATE TRIGGER operations_set_updated_at    BEFORE UPDATE ON operations
  FOR EACH ROW EXECUTE FUNCTION trg_set_updated_at();
...
CREATE TRIGGER persons_set_updated_at       BEFORE UPDATE ON persons
  FOR EACH ROW EXECUTE FUNCTION trg_set_updated_at();
CREATE TRIGGER animals_set_updated_at       BEFORE UPDATE ON animals
  FOR EACH ROW EXECUTE FUNCTION trg_set_updated_at();
...
ALTER TABLE translations
  DROP CONSTRAINT translations_operation_id_operations_id_fk;
...
ALTER TABLE translations
  DROP CONSTRAINT translations_locale_locales_code_fk;
ALTER TABLE translations
...
ALTER TABLE audit_entries
  DROP CONSTRAINT audit_entries_operation_id_operations_id_fk;
...
CREATE INDEX user_operation_roles_op_id_idx       ON user_operation_roles (operation_id);

CREATE INDEX sectors_op_id_idx                    ON sectors              (operation_id);

CREATE INDEX sites_op_id_idx                      ON sites                (operation_id);
CREATE INDEX sites_sector_id_idx                  ON sites                (sector_id);
CREATE INDEX sites_lifecycle_idx                  ON sites                (operation_id, lifecycle_state);

CREATE INDEX persons_op_id_idx                    ON persons              (operation_id);

CREATE INDEX animals_op_id_idx                    ON animals              (operation_id);
CREATE INDEX animals_current_site_id_idx          ON animals              (current_site_id);
CREATE INDEX animals_lifecycle_idx                ON animals              (operation_id, lifecycle_state);

CREATE INDEX sightings_op_id_idx                  ON sightings            (operation_id);
CREATE INDEX sightings_site_id_idx                ON sightings            (site_id);
CREATE INDEX sightings_animal_id_idx              ON sightings            (observed_animal_id);
CREATE INDEX sightings_person_id_idx              ON sightings            (reported_by_person_id);
CREATE INDEX sightings_ts_idx                     ON sightings            (operation_id, ts DESC);

CREATE INDEX leads_op_id_idx                      ON leads                (operation_id);
CREATE INDEX leads_lifecycle_idx                  ON leads                (operation_id, lifecycle_state);
CREATE INDEX leads_assignee_idx                   ON leads                (assigned_to_user_id);
CREATE INDEX leads_site_id_idx                    ON leads                (site_id);
CREATE INDEX leads_priority_idx                   ON leads                (operation_id, priority_score DESC);

CREATE INDEX trap_events_op_id_idx                ON trap_events          (operation_id);
CREATE INDEX trap_events_lead_id_idx              ON trap_events          (lead_id);
CREATE INDEX trap_events_site_id_idx              ON trap_events          (site_id);
CREATE INDEX trap_events_animal_id_idx            ON trap_events          (trapped_animal_id);

CREATE INDEX medical_events_op_id_idx             ON medical_events       (operation_id);
CREATE INDEX medical_events_animal_id_idx         ON medical_events       (animal_id);
CREATE INDEX medical_events_vet_id_idx            ON medical_events       (vet_person_id);
CREATE INDEX medical_events_ts_idx                ON medical_events       (operation_id, ts DESC);

CREATE INDEX handovers_op_id_idx                  ON handovers            (operation_id);
CREATE INDEX handovers_site_id_idx                ON handovers            (site_id);

CREATE INDEX translations_locale_idx              ON translations         (locale);
CREATE INDEX translations_org_id_idx              ON translations         (organization_id);
CREATE INDEX translations_op_id_idx               ON translations         (operation_id);
